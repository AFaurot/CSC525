...
def inference(opts):
    ...
    while True:
        ...
        tgt_tokens = greedy_decode(
            model, src, src_mask, max_len=num_tokens + 5, start_symbol=special_symbols["<bos>"],
...
def train(model, train_dl, loss_fn, optim, special_symbols, opts):
    ...
    for src, tgt in tqdm(train_dl, ascii=True):
        ...
        # Create masks
        src_mask, tgt_mask, src_padding_mask, tgt_padding_mask = create_mask(src, tgt_input, special_symbols["<pad>"],
                                                                             DEVICE)
